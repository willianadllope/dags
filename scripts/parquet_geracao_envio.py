...
def export_query_to_parquet(sql,tipo, fileprefix, limit, strposicao='001'):
    ...
    for i, df in enumerate(pd.read_sql(sql, con, chunksize=limit)):
        ...
        file_name = '/parquet2/tabelao/'+tipo+'/'+fileprefix+'/'+fileprefix + '_'+strposicao + '_'+str(i) +'_'+ formatted_previous_day+'.parquet'   
...
def delete_files_directory(tipo, diretorio):
    ...
    directory_path = '/parquet2/tabelao/'+tipo+'/'+diretorio
...
def send_parquet_snowflake(tipo, tabela):
    ...
    file_name = '/parquet2/tabelao/'+tipo+'/'+tabela+'/*'
    STAGE_SCHEMA = 'FULL'
    if(tipo=='INCREMENTAL'):
